...
class AlexNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x) # 1->96
        x = self.conv2(x) # 96->256
        x = self.conv3(x) # 256 -> 384 -> 256
        x = self.fc1(x) # 32 * 256 * 5 * 5 --> 32 * 4096
        x = self.fc2(x) # 4096 -> 4096
        x = self.fc3(x) # 4096 -> 10
        return x
...
for step in range(epoch):
    ...
    scheduler.step()
    # print(f"训练周期: {step}, 误差: {train_loss / len(train_dataloader)}, 准确度: {train_acc / len(mnist_train)}")
    ...
    print(f"周期: {step}, 误差: {test_loss / len(test_dataloader)}, 准确度: {test_acc / len(mnist_test)}")
    # 统计
